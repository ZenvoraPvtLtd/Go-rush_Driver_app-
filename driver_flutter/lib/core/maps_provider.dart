...
class _LightMapViewState extends State<LightMapView> with SingleTickerProviderStateMixin {
        ...
        child: Stack(
          ...
          children: [
            AnimatedBuilder(
              ...
              builder: (context, _) {
                return CustomPaint(
                  painter: _LightMapPainter(pulse: _pulseController.value, showRoute: widget.showDestination),
                );
              },
            ),
            ...
            Center(
              child: Container(
                ...
                child: Row(
                  ...
                  children: [
                    const Icon(Icons.location_on, color: QuickServeColors.primaryOrange, size: 16),
                    const SizedBox(width: 4),
                    Text(
                      widget.locationTitle,
                      style: const TextStyle(color: QuickServeColors.textDark, fontSize: 12, fontWeight: FontWeight.bold),
                    ),
                  ],
                ),
              ),
...
class _LightNavigationMapViewState extends State<LightNavigationMapView> with SingleTickerProviderStateMixin {
  late AnimationController _carPulse;

  @override
  void initState() {
    super.initState();
    _carPulse = AnimationController(vsync: this, duration: const Duration(seconds: 2))..repeat();
  }
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: AnimatedBuilder(
        ...
        builder: (context, _) {
          return CustomPaint(
            painter: _LightNavMapPainter(
              carPulse: _carPulse.value,
...
class _LightMapPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTWH(size.width * 0.1, size.height * 0.15, size.width * 0.28, size.height * 0.4), const Radius.circular(8)),
      parkPaint,
    );
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTWH(size.width * 0.65, size.height * 0.45, size.width * 0.3, size.height * 0.45), const Radius.circular(8)),
      parkPaint,
    );
    ...
    avenuePath.quadraticBezierTo(size.width * 0.45, size.height * 0.5, size.width, size.height * 0.2);
    ...
    canvas.drawCircle(Offset(size.width * 0.5, size.height * 0.5), 18 + (pulse * 24), pulsePaint);
  }
...
class _LightNavMapPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final parkPaint = Paint()..color = const Color(0xFFE8F3E8);
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTWH(size.width * 0.05, size.height * 0.1, size.width * 0.35, size.height * 0.25), const Radius.circular(12)),
      parkPaint,
    );
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTWH(size.width * 0.6, size.height * 0.55, size.width * 0.35, size.height * 0.25), const Radius.circular(12)),
      parkPaint,
    );

    // Roads
    final roadBorder = Paint()..color = const Color(0xFFE2E0D5)..strokeWidth = 8.0..style = PaintingStyle.stroke;
    final roadFill = Paint()..color = Colors.white..strokeWidth = 6.0..style = PaintingStyle.stroke;
    ...
    final carPulsePaint = Paint()
      ..color = QuickServeColors.primaryOrange.withOpacity((1.0 - carPulse) * 0.4)
      ..style = PaintingStyle.fill;
    ...
    canvas.drawCircle(carPos, 14, Paint()..color = Colors.white..style = PaintingStyle.stroke..strokeWidth = 2);
    ...
    iconPainter.paint(canvas, carPos - Offset(iconPainter.width / 2, iconPainter.height / 2));
  }
