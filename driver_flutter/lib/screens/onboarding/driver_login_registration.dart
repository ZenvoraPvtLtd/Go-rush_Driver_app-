...
class _DriverLoginRegistrationScreenState extends State<DriverLoginRegistrationScreen> {
  ...
  Future<void> _handleRegister() async {
    ...
    if (email.isEmpty || !emailRegex.hasMatch(email)) {
      AppToast.error(context, 'Please enter a valid email address (e.g., name@example.com)');
      return;
    }
    ...
    try {
      debugPrint('[Register] Calling API with name=$name, phone=$phoneDigits, email=$email');
      ...
      if (result.success) {
        ...
        final effectiveToken = result.token ?? 'session_${DateTime.now().millisecondsSinceEpoch}';
  ...
  Future<void> _handleLogin() async {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (mounted) {
          AppToast.success(context, result.message.isNotEmpty ? result.message : 'Login successful!');
        }
        ...
        AppToast.error(context, result.message.isNotEmpty ? result.message : 'Invalid credentials');
      }
    } catch (e) {
      if (mounted) {
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: 24),
                child: _isRegisterMode ? _buildRegisterFields() : _buildLoginFields(),
              ),
