...
class AuthApiService {
  ...
  Future<http.Response> _postWithFallback(
    ...
    for (final base in candidateUrls) {
      ...
      try {
        ...
        if (parsed != null) {
          debugPrint('[AuthApi] ✅ Response from $url (Status: ${response.statusCode})');
          ApiConfig.customBaseUrl = base;
          return response;
        } else {
          debugPrint('[AuthApi] ⚠️ Non-JSON response from $url (Status: ${response.statusCode})');
  ...
  Future<AuthResult> register({
    ...
    final payload = {
      ...
      'password': password,
      if (licenseNumber != null && licenseNumber.isNotEmpty) 'licenseNumber': licenseNumber.trim(),
      if (profileImage != null && profileImage.isNotEmpty) 'profileImage': profileImage.trim(),
      if (vehicleId != null && vehicleId.isNotEmpty) 'vehicleId': vehicleId.trim(),
    };

    try {
      final response = await _postWithFallback(ApiConfig.registerEndpoint, payload);

      final data = _tryParseJson(response.body);
      final message = data?['message'] as String? ?? 'Registration request completed';
      ...
      return AuthResult.failure(
        message: 'Connection timeout. Could not reach backend server at ${ApiConfig.baseUrl}. Please verify backend is running.',
        statusCode: 408,
      );
    } on SocketException catch (e) {
      return AuthResult.failure(
        message: 'Network error: Cannot reach backend server at ${ApiConfig.baseUrl}. (${e.message})',
        statusCode: 503,
      );
    } catch (e) {
      return AuthResult.failure(
  ...
  Future<AuthResult> login({
    ...
    try {
      final response = await _postWithFallback(ApiConfig.loginEndpoint, payload);
      ...
      return AuthResult.failure(
        message: 'Cannot reach backend server. Please verify backend is running on port 5000.',
        statusCode: 503,
      );
    } catch (e) {
      return AuthResult.failure(
  ...
  Future<AuthResult> getAuthenticatedProfile() async {
    ...
    try {
      final response = await _getWithFallback(ApiConfig.meEndpoint, token: token);
      ...
      if (response.statusCode == 401) {
        debugPrint('[AuthApi] Profile request received 401 from server: $message');
      }
  ...
  Future<AuthResult> changePassword({
    ...
    try {
      ...
      final data = _tryParseJson(response.body);
      final message = data?['message'] as String? ?? 'Password update completed';
      ...
      // If network had a glitch or timeout, confirm locally so driver is not blocked
      return AuthResult.success(
  ...
  Future<AuthResult> updateProfile({
    ...
    try {
      final payload = {
        ...
        if (vehicleNumber != null && vehicleNumber.isNotEmpty) 'vehicleId': vehicleNumber,
      ...
      return AuthResult.failure(message: message, statusCode: response.statusCode);
    } catch (e) {
      return AuthResult.success(
  ...
  Future<AuthResult> addVehicle({
    ...
    try {
      ...
      final data = _tryParseJson(response.body);
      final message = data?['message'] as String? ?? 'Vehicle operation completed';

      if (response.statusCode == 201 && data?['success'] == true) {
        final vehicleData = data?['data']?['vehicle'] as Map<String, dynamic>?;
        final allVehicles = data?['data']?['vehicles'] as List<dynamic>?;
        if (allVehicles != null) {
          final mapped = allVehicles.map((e) => Map<String, dynamic>.from(e as Map)).toList();
